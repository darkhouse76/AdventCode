...
namespace CodeTAF
{
    public class AoC7 : MonoBehaviour
    {
        ...
        int GetSizeExtra(string targetDir) {
            int totalSize = 0;
            for (int i = 0; i < fileData.Count; i++) {
                if (fileData[i].rootDir == targetDir) {
                    if (fileData[i].isDir) {
                        totalSize += GetSizeExtra(fileData[i].rootDir + fileData[i].name);
                        continue;
                    }
        ...
        int GetHowManyThings(string targetDir) {
            int count = 0;
            for (int i = 0; i < fileData.Count; i++) {
                if (fileData[i].rootDir == targetDir) { count++; }
            }
