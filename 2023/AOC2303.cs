...
namespace CodeTAF
{
    public class AOC2303 : MonoBehaviour
    {
        [SerializeField]
        ...
        int FindNumber(string inputLine, int numIndex) {
            ...
            while (char.IsDigit(inputLine[numIndex+curIndex])) {
                curIndex--;
                if ((numIndex + curIndex) < 0) { break; }
                
            }

            minIndex = curIndex + 1 + numIndex;
            curIndex = 0;

            while (char.IsDigit(inputLine[numIndex + curIndex])) {
                curIndex++;
                if ((numIndex + curIndex) == inputLine.Length) { break; }
                
            }
            maxIndex = curIndex + numIndex;

            //print(inputLine.Substring(minIndex, maxIndex - minIndex));
            return int.Parse(inputLine.Substring(minIndex, maxIndex - minIndex));
                        ...
                        for (int k = -1; k < 2; k++) { //checking row above, at, and below the found symbol
                            ...
                            for (int l = 0; l < testString.Length; l++) { 
                                if (char.IsDigit(testString[l]) ) {
                                    ...
                                    int foundNumber = FindNumber(engine[i + k], (j - 1) + l);
