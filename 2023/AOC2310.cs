...
namespace CodeTAF
{
    public class AOC2310 : MonoBehaviour
    {
        [SerializeField]
        ...
        char[,] ParseMaze(out Vector2Int mazeStart) {
            // 0,0 is in the bottom left of the maze
            string[] mazeLines = input.Split("\r\n");
            char[,] maze = new char[mazeLines[0].Length,mazeLines.Length];
            mazeStart = Vector2Int.zero;
            Array.Reverse<string>(mazeLines);
            for (int line = 0; line < mazeLines.Length; line++) {                
                for (int i = 0; i < mazeLines[line].Length; i++) {
                    string curLine = mazeLines[line];
                    maze[i,line] = mazeLines[line][i];
                    if (mazeLines[line][i] == 'S') { mazeStart = new Vector2Int(i,line); } 
                }
            }
            return maze;
        }
        ...
        Vector2Int GetDirection(char pipe, Vector2Int lastDir) {      
            
            switch (pipe) {
                case '|':                    
                    return new Vector2Int(0,lastDir.y);                    
                case '-':                                     
                    return new Vector2Int(lastDir.x, 0);                    
                case 'L':
                    return (lastDir == Vector2Int.left) ? Vector2Int.up : Vector2Int.right;                                    
                case 'J':
                    return (lastDir == Vector2Int.right) ? Vector2Int.up : Vector2Int.left;                                       
                case '7':
                    return (lastDir == Vector2Int.right) ? Vector2Int.down : Vector2Int.left;                                     
                case 'F':
                    return (lastDir == Vector2Int.left) ? Vector2Int.down : Vector2Int.right;                                       
                case '.':
                    print("ground found");
                    return errorVector;
                case 'S':
                ...
                default:
                    Debug.LogWarning($"INVALD PIPE encountered! pipe= {pipe}");
                    return errorVector;
            }
        ...
        Vector2Int[] part1() {
            ...
            for (int i = 0; i < startDir.Length; i++) {
                ...
                } while (stepsCount < 100000000);
                //if (highestCount > 0) { break; }
            }
